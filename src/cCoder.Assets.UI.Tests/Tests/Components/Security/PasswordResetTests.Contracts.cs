...
{
    [Fact]
    public void ForgotPasswordTemplate_ShouldAppendResetPathToCanonicalAppRoot()
    {
        // Given
        ...
        using JsonDocument template = JsonDocument.Parse(
            json: File.ReadAllText(path: templatePath));

        string rawString = template.RootElement
            .GetProperty(propertyName: "RawString")
            .GetString()!;

        // Then
        Assert.Contains(
            expectedSubstring: "href=\"[app[root]]ResetPassword?",
            actualString: rawString,
