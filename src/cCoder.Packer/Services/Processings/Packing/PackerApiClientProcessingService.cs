...
internal sealed partial class PackerApiClientProcessingService(HttpClient httpClient, Uri source)
    : IPackerApiClientProcessingService
{
    ...
    public Task LoginAsync(
        ...
        {
            Validate(inputs: [user, password, cancellationToken]);
...
message: "The login response did not contain a bearer token.");
    ...
    public Task<IReadOnlyList<ExportRecord>> ExportCommonCacheAsync(
        CancellationToken cancellationToken = default) =>
        TryCatch(operation: async () =>
        {
            Validate(inputs: cancellationToken);

            return await ExportCommonCacheInternalAsync(
                cancellationToken: cancellationToken);
        });

    private async Task<IReadOnlyList<ExportRecord>> ExportCommonCacheInternalAsync(
        CancellationToken cancellationToken)
    {
...
value: value.RootElement.Clone()));
    ...
    public Task<IReadOnlyList<ExportRecord>> ExportAppAsync(
        int? requestedAppId,
        CancellationToken cancellationToken = default) =>
        TryCatch(operation: async () =>
        {
            Validate(inputs: [requestedAppId, cancellationToken]);

            return await ExportAppInternalAsync(
                requestedAppId: requestedAppId,
                cancellationToken: cancellationToken);
        });

    private async Task<IReadOnlyList<ExportRecord>> ExportAppInternalAsync(
        int? requestedAppId,
        CancellationToken cancellationToken = default)
    {
...
cancellationToken: cancellationToken);
        ...
        JsonElement[] apps =
        [
            .. GetProperty(value: response.RootElement, name: "value")
                .EnumerateArray(),
        ];
